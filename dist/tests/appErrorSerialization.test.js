...
describe('AppError JSON Serialization & Error Middleware Verification', () => {
    ...
    const errorInstances = [
        new AppError_1.ValidationError('Invalid email format', { field: 'email' }),
        new AppError_1.AuthenticationError('Invalid or expired JWT token'),
        new AppError_1.NotFoundError('Requested metric entry not found'),
        new AppError_1.ExternalServiceError('GoogleHealthAPI', 'Upstream gateway timeout', 504),
        new AppError_1.DatabaseError('Database connection pool exhausted'),
        new AppError_1.CryptographicError('AES-256-GCM authentication tag mismatch'),
    ];
    ...
    test('Express error handling middleware serializes AppError subclass with correct message and status code', async () => {
        ...
        testApp.get('/test-error/:type', (req, res, next) => {
            switch (req.params.type) {
                case 'validation':
                    return next(new AppError_1.ValidationError('Invalid parameter input'));
                case 'auth':
                    return next(new AppError_1.AuthenticationError('Unauthorized access'));
                case 'notfound':
                    return next(new AppError_1.NotFoundError('Resource missing'));
                case 'external':
                    return next(new AppError_1.ExternalServiceError('GoogleAPI', 'Token exchange failed', 400));
                case 'database':
                    return next(new AppError_1.DatabaseError('Query execution failed'));
                case 'crypto':
                    return next(new AppError_1.CryptographicError('Decryption failed'));
                default:
                    return next(new AppError_1.AppError('Generic app error', 500, 'GENERIC_ERROR'));
            }
        });
        testApp.use(app_1.errorHandlerMiddleware);
        const routesToTest = [
        ...
        for (const route of routesToTest) {
            const res = await (0, supertest_1.default)(testApp).get(route.path);
    ...
    test('Omits context from AppError and details from unhandled errors when NODE_ENV=production', async () => {
        ...
        const testApp = (0, express_1.default)();
        testApp.use(express_1.default.json());
        testApp.get('/prod-app-error', (_req, _res, next) => {
            next(new AppError_1.ValidationError('Validation failed', { sensitiveInternalId: 998877 }));
        });
        testApp.get('/prod-unhandled-error', (_req, _res, next) => {
        ...
        const appErrRes = await (0, supertest_1.default)(testApp).get('/prod-app-error');
        ...
        const unhandledRes = await (0, supertest_1.default)(testApp).get('/prod-unhandled-error');
    ...
    test('Catch-all branch hides real error message and returns generic Internal server error when NODE_ENV=production', async () => {
        ...
        testApp.get('/throw-plain-error', (_req, _res, next) => {
            next(new Error(sensitiveErrorMsg));
        });
        testApp.use(app_1.errorHandlerMiddleware);
        const res = await (0, supertest_1.default)(testApp).get('/throw-plain-error');
