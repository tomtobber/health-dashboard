...
- Snapshots are generated by `POST /api/metrics/:metricType/baseline-history/refresh` (or account-wide `POST /api/metrics/baseline-history/refresh`), which is idempotent and safely re-callable; it processes a bounded amount of work per call (50 snapshots/call) rather than looping unboundedly. Baseline panels execute the refresh targeting their specific metric in a `while (hasMore)` loop until complete.
