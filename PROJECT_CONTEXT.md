...
- Snapshots are generated by `POST /api/metrics/baseline-history/refresh`, which is idempotent and safely re-callable; it processes a bounded amount of work per call (50 snapshots/call) rather than looping unboundedly. Future frontend refresh handlers must call this in a `while (hasMore)` loop until complete.
